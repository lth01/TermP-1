...
public class ClientDAO { 					//DAO는 DB변경 메소드 집합
	...
	public static boolean createroom(String uid, String roomName){
		...
		try{
			...
			pstmt = conn.prepareStatement("insert into participants(id,client_id,room_id,entry) values(seq_Participants.nextval,?,?,sysdate)");
	...
	public static StringBuffer showroom(String id){
		...
		try{
			...
			rs = stmt.executeQuery("select R.room_name, R.id from room R JOIN PARTICIPANTS p ON R.id = room_id WHERE client_id='"+id+"'" );
	...
	public static StringBuffer enterroom(String room_id,String uid){
		...
		try{
			rs = stmt.executeQuery("select c.name,m.message,m.datemessage FROM CLIENT c \n"
					...
					+ "and datemessage >=(select entry from participants p where p.room_id = '"+room_id+"' and p.client_id ='"+uid+"' )\n"
		...
		try{
			...
			if(result>=1){
				...
				while(rs.next()){
					...
					ResultSet rs2 = stmt2.executeQuery("select ip from Client c where c.id = "+"\'"+Clientid+"\'");
					rs2.next();
